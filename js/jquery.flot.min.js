...
(function (b) {
  ...
  b.color.make = function (d, e, g, f) {
    ...
    c.a = f != null ? f : 1;
    c.add = function (h, j) {
      for (var k = 0; k < h.length; ++k) {c[h.charAt(k)] += j}
      return c.normalize()
    };
  ...
  b.color.parse = function (c) {
    ...
    if (e == "transparent") {return f(255, 255, 255, 0)} else {
      d = a[e] || [0, 0, 0];
...
(function (c) {
  function b(av, ai, J, af) {
    ...
    function an(aD, aB) {
      aB = [aq].concat(aB);
      for (var aC = 0; aC < aD.length; ++aC) {aD[aC].apply(this, aB)}
    }
    ...
    function ax() {
      ...
      for (aG = 0; aG < Q.length; ++aG) {
        ...
        if (aJ != null) {
          --aM;
          if (typeof aJ == "number") {aE.push(aJ)} else {aB.push(c.color.parse(Q[aG].color))}
        }
      }
    ...
    function z() {
      ...
      function aF(a3, a2, a1) {
        if (a2 < a3.datamin && a2 != -aB) {a3.datamin = a2}
            ...
            for (aN = 0; aN < aP; ++aN) {
              ...
              if (aX) {
                if (aX.number && a0 != null) {
                  a0 = +a0;
                  if (isNaN(a0)) {a0 = null} else {if (a0 == Infinity) {a0 = aB} else {if (a0 == -Infinity) {a0 = -aB}}}
                }
                if (a0 == null) {
                  if (aX.required) {aE = true}
      ...
      c.each(m(), function (a1, a2) {
        if (a2.datamin == aO) {a2.datamin = null}
    ...
    function t() {
      ...
      if (O.grid.show) {
        ...
        if (aD == null) {
          aD = 0;
          for (aC = 0; aC < Q.length; ++aC) {aD = Math.max(aD, Q[aC].points.radius + Q[aC].points.lineWidth / 2)}
        }
        for (var aB in q) {
    ...
    function S(aG) {
      ...
      if (aM.mode == "time") {
        ...
        aR = function (aV, aY) {
          ...
          if (aW < aJ.minute) {fmt = "%h:%M:%S" + aZ} else {if (aW < aJ.day) {if (aX < 2 * aJ.day) {fmt = "%h:%M" + aZ} else {fmt = "%b %d %h:%M" + aZ}} else {if (aW < aJ.month) {fmt = "%b %d"} else {if (aW < aJ.year) {if (aX < aJ.year) {fmt = "%b"} else {fmt = "%b %y"}} else {fmt = "%y"}}}}
        ...
        if (aI < 1.5) {aO = 1} else {
          ...
          } else {if (aI < 7.5) {aO = 5} else {aO = 10}}
        }
        aO *= aQ;
      ...
      if (aM.alignTicksWithAxis != null) {
        ...
        if (aF && aF.used && aF != aG) {
          var aL = aB(aG);
          if (aL.length > 0) {
            if (aM.min == null) {aG.min = Math.min(aG.min, aL[0])}
    ...
    function ap(aB, aC) {
      if (aB.options.autoscaleMargin && aC.length > 0) {
        if (aB.options.min == null) {aB.min = Math.min(aB.min, aC[0].v)}
    ...
    function ac() {
      ...
      for (var aE = 0; aE < aK.length; ++aE) {
        ...
        if (aB.direction == "x") {
          aN = 0;
          if (aQ == "full") {aL = (aB.position == "top" ? 0 : w)} else {aL = aG.top - q.top + (aB.position == "top" ? aG.height : 0)}
        } else {
          aL = 0;
          if (aQ == "full") {aN = (aB.position == "left" ? 0 : h)} else {aN = aG.left - q.left + (aB.position == "left" ? aG.width : 0)}
        }
        if (!aB.innermost) {
          H.beginPath();
          aP = aJ = 0;
    ...
    function s() {
      ...
      for (aC = 0; aC < ab.length; ++aC) {
        aB = ab[aC];
        if (aB.series.bars.show) {v(aB.series, aB.point)} else {ay(aB.series, aB.point)}
      }
      A.restore();
      an(ak.drawOverlay, [A])
    }
