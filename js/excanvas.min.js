  ...
  (function () {
    ...
    var E = {
      init:           function (Z) {
        ...
        for (var Z = 0; Z < j.length; Z++) {this.initElement(j[Z])}
    ...
    function V() {return [[1, 0, 0], [0, 1, 0], [0, 0, 1]]}
    ...
    function g(i) {
      ...
      if (j.length == 4 && i.substr(3, 1) == "a") {alpha = Number(j[3])} else {j[3] = 1}
      return j
    }
    ...
    function A(i, Z, j) {
      if (j < 0) {j++}
      if (j > 1) {j--}
      if (6 * j < 1) {return i + (Z - i) * 6 * j} else {if (2 * j < 1) {return Z} else {if (3 * j < 2) {return i + (Z - i) * (2 / 3 - j) * 6} else {return i}}}
    }
    ...
    M.stroke = function (AM) {
      ...
      for (var AH = 0; AH < this.currentPath_.length; AH += AE) {
        ...
        for (var AI = AH; AI < Math.min(AH + AE, this.currentPath_.length); AI++) {
          ...
          if (Z) {
            if (AG.x == null || Z.x < AG.x) {AG.x = Z.x}
            if (AL.x == null || Z.x > AL.x) {AL.x = Z.x}
            if (AG.y == null || Z.y < AG.y) {AG.y = Z.y}
            if (AL.y == null || Z.y > AL.y) {AL.y = Z.y}
          }
        }
        AK.push(' ">');
    ...
    function q(Z) {
      if (!Z || Z.nodeType != 1 || Z.tagName != "IMG") {n("TYPE_MISMATCH_ERR")}
      if (Z.readyState != "complete") {n("INVALID_STATE_ERR")}
    }
