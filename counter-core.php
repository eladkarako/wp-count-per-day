...
JSEND;
  ...
  function export($days = 180) {
    ...
    if (!$f) :
      ...
      $part = min([round($freeMemory / 1000000) * 500, $part]);
      $start = 0;
  ...
  function restore() {
    ...
    if (isset($path) && preg_match('/count_per_day|cpd_counter/i', $path) && file_exists($path)) {
      ...
      if (strpos($path, 'counter_backup')) {
        // counter table
        $cpd_sep = ['DROP TABLE', 'CREATE TABLE', 'INSERT INTO', 'REPAIR TABLE'];
        $sql = '';
        ...
        $entries = ['count_per_day', 'count_per_day_summary', 'count_per_day_collected', 'count_per_day_posts', 'count_per_day_notes'];
  ...
  function getSearchString() {
    ...
    foreach ($keys as $key)
      if (isset($query[ $key ]))
        $search = str_ireplace(['/search?q=', '/images?q='], '', $query[ $key ]);
    if (isset($search))
